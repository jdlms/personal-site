<script lang="ts">
	import type { PostModule, PostEntry } from '$lib/types/post';

	const postModules = import.meta.glob('$content/blog/*.md', { eager: true });

	const posts: PostEntry[] = Object.entries(postModules)
		.map(([path, module]) => {
			const { metadata } = module as PostModule;
			const slug = path.split('/').pop()?.replace('.md', '') ?? '';
			const dateValue = metadata.pubDatetime ?? metadata.date;
			const date = dateValue ? new Date(dateValue) : null;

			return {
				slug,
				title: metadata.title ?? slug,
				date,
				description: metadata.description
			};
		})
		.sort((a, b) => (b.date?.getTime() ?? 0) - (a.date?.getTime() ?? 0));
</script>

<div class="flex flex-col justify-center pt-12 items-center text-text-heading">
	<div class=" text-text-heading mb-12 mt-4 flex flex-col gap-6">
		{#if posts.length === 0}
			<p class="text-text-muted">No posts yet.</p>
		{:else}
			{#each posts as post}
				<a
					href={`/post/${post.slug}`}
					class="flex flex-row font-mono text-xs space-x-12 md:space-x-24 items-center justify-around bg-desk-background p-4 rounded-md"
				>
					<svg
						width="16"
						height="16"
						viewBox="0 0 24 24"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
					>
						<path
							d="M6 6C6 5.44772 6.44772 5 7 5H17C17.5523 5 18 5.44772 18 6C18 6.55228 17.5523 7 17 7H7C6.44771 7 6 6.55228 6 6Z"
							fill="currentColor"
						/>
						<path
							d="M6 10C6 9.44771 6.44772 9 7 9H17C17.5523 9 18 9.44771 18 10C18 10.5523 17.5523 11 17 11H7C6.44771 11 6 10.5523 6 10Z"
							fill="currentColor"
						/>
						<path
							d="M7 13C6.44772 13 6 13.4477 6 14C6 14.5523 6.44771 15 7 15H17C17.5523 15 18 14.5523 18 14C18 13.4477 17.5523 13 17 13H7Z"
							fill="currentColor"
						/>
						<path
							d="M6 18C6 17.4477 6.44772 17 7 17H11C11.5523 17 12 17.4477 12 18C12 18.5523 11.5523 19 11 19H7C6.44772 19 6 18.5523 6 18Z"
							fill="currentColor"
						/>
						<path
							fill-rule="evenodd"
							clip-rule="evenodd"
							d="M2 4C2 2.34315 3.34315 1 5 1H19C20.6569 1 22 2.34315 22 4V20C22 21.6569 20.6569 23 19 23H5C3.34315 23 2 21.6569 2 20V4ZM5 3H19C19.5523 3 20 3.44771 20 4V20C20 20.5523 19.5523 21 19 21H5C4.44772 21 4 20.5523 4 20V4C4 3.44772 4.44771 3 5 3Z"
							fill="currentColor"
						/>
					</svg>
					<p>{post.title}</p>
					<span class="inline-block text-link text-xs font-semibold m-0">
						{post.date ? post.date.toLocaleDateString() : ''}
					</span>
				</a>
			{/each}
		{/if}
	</div>
</div>
